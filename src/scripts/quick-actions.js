...
export function setupQuickActions(container, data, handlers = {}) {
  ...
  function render(state) {
    ...
    // Цена текущего окна + кнопка «В корзину».
    // Цена показывается ЗДЕСЬ (за настраиваемое окно), а не в корзине —
    // окно ещё не добавлено, поэтому в корзине это была бы неверная сумма.
    const currency = data.meta?.currency ?? '₽';
    const { total } = calcPrice(state, data);
    container.append(
      el('div', { className: 'quick__actions' }, [
        el('div', { className: 'quick__price' }, [
          el('span', { className: 'quick__price-label', textContent: 'Цена окна' }),
          el('span', {
            className: 'quick__price-value',
            textContent: formatPrice(total, currency),
          }),
        ]),
        el('button', {
