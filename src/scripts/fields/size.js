...
function createSizeCard({ name, min, max, value, onInput }) {
  ...
  // Установить значение извне (напр. из размерного input на превью)
  // без вызова onInput — чтобы не зациклить синхронизацию.
  function setValue(v) {
    apply(v, false, true);
  }

  return { card, setRange, setValue };
}
...
export function createSizeField(type, onChange) {
  ...
  // Синхронизация со state (напр. значения ввели на превью).
  // Обновляет карточки без onChange, чтобы не зациклить.
  function syncFromState(w, h) {
    if (w != null && w !== size.width) {
      size.width = w;
      widthCard.setValue(w);
    }
    if (h != null && h !== size.height) {
      size.height = h;
      heightCard.setValue(h);
    }
  }

  return { field, setType, getSize: () => ({ ...size }), syncFromState };
}
