...
export function createOpeningField(type, openingTypes, onChange) {
  ...
  /**
   * Синхронизирует radio с внешним состоянием (напр. изменили в quick actions).
   * Не вызывает onChange, чтобы не зациклить обновления.
   * @param {string[]} externalOpenings
   */
  function syncFromState(externalOpenings) {
    externalOpenings.forEach((id, i) => {
      if (openings[i] === id) return;
      openings[i] = id;
      const input = groupsWrap.querySelector(`input[name="opening-${i}"][value="${id}"]`);
      if (input) input.checked = true;
    });
  }

  return { field, setType, getOpenings: () => [...openings], syncFromState };
}
