...
async function submitOrder(payload) {
  // --- ЗАГЛУШКА (удалить при подключении PHP) ---
  console.log('[checkout] payload для отправки:', payload);
  await new Promise((r) => setTimeout(r, 600)); // эмуляция сети
  ...
  form.addEventListener('submit', async (e) => {
    ...
    const payload = {
      ...
      customer: {
        ...
        messengerContact: (data.messengerContact || '').trim(),
