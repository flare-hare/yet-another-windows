 ...
 */
/**
 * Создаёт размерную линию (ножки по краям + пунктир + значение).
 * Значение только для показа (редактируется слайдерами в панели).
 * @param {Object} cfg
 * @param {'width'|'height'} cfg.axis
 * @param {number} cfg.value - значение, см
 * @returns {HTMLElement}
 */
function createDimLine({ axis, value }) {
  const line = el('div', { className: `win-dim win-dim--${axis}` });

  // Ножка на старте + пунктирная линия + ножка на конце
  line.append(el('span', { className: 'win-dim__leg' }));
  line.append(el('span', { className: 'win-dim__line' }));
  line.append(el('span', { className: 'win-dim__leg' }));

  // Значение в акцентной «таблетке» по центру
  const valueBox = el('span', {
    className: 'win-dim__value',
    textContent: `${value} см`,
    attrs: {
      'aria-label': axis === 'width' ? `Ширина ${value} см` : `Высота ${value} см`,
    },
  });
...
function buildWindowEl({
  ...
  // Размерные линии (только показ; редактирование — слайдерами в панели)
  const widthDim = createDimLine({ axis: 'width', value: width });
  const heightDim = createDimLine({ axis: 'height', value: height });

  // Линии крепим к обёртке РАМЫ (а не всей сцены) — тогда они точно
  // совпадают с гранями окна независимо от gap и ряда шестерён снизу.
  const frameBox = el('div', { className: 'win-frame-box' }, [widthDim, frame, heightDim]);

  // Рама с линиями + отдельный ряд шестерён в колонке-сцене
  const stage = el('div', { className: 'win-stage' }, [frameBox, gears]);
  wrap.append(stage);
  return wrap;
}
